...
class TelemetreeClient {
	constructor(projectId, apiKey, options = {}) {
		this.logger = new Logger(options.logging || {});
		this.settings = new Config(projectId, apiKey, this.logger);
	}

	async initialize() {
		await this.settings.initialize();
		this.encryption = new EncryptionService(
			this.settings.config.publicKey,
			this.logger,
		);
		this.eventBuilder = new EventBuilder(this.settings);
		this.httpClient = new HttpClient(this.settings, this.logger);
	}
	...
	async track(user, eventName, eventProperties = {}) {
		...
		const eventData = this.eventBuilder.buildPayload(
			user,
			eventName,
			eventProperties,
		);
		...
		try {
			...
			if (!response.success) {
				this.logger.error("Failed to track event:", response);
				throw new Error(
	...
	async trackUpdate(updateData) {
		...
		const eventData = this.eventBuilder.buildPayloadUpdate(
			updateData,
			eventType,
		);
		const encryptedData = await this.encryption.encrypt(eventData);
		try {
			...
			if (!response.success) {
				this.logger.error("Failed to track update:", response);
				throw new Error(
					`Tracking failed: ${response.error || "Unknown error"}`,
				);
			}
			...
			this.logger.error("Failed to track update:", error);
			throw error;
		}
	}
}
